<script lang="ts" module>
	export const columns: ColumnDef<History>[] = [
		...
		},
		{
			id: 'actions',
			header: 'Action',
			cell: ({ row }) => renderSnippet(DataTableAction, { row }),
			size: 120
		}
	];
</script>
...
<div class="flex w-full flex-col justify-start gap-6">
	<div class="flex flex-col gap-2 px-4 lg:px-6">
		<h1 class="text-3xl font-semibold tracking-tight transition-colors">Riwayat Pesanan</h1>
		<p class="text-muted-foreground text-sm">Lihat semua riwayat pesanan yang pernah Anda buat.</p>
	...
	<div class="flex items-center justify-between px-6 lg:px-10">
		<div class="text-muted-foreground hidden flex-1 text-sm lg:flex">
			Showing {data.pagination?.totalCount || 0} total orders.
		</div>
		<div class="flex w-full items-center gap-8 lg:w-fit">
			<div class="hidden items-center gap-2 lg:flex">
				<Label for="rows-per-page" class="text-sm font-medium"
					>Rows per page {data.pagination?.limit || 10}</Label
				>
			</div>
			...
			<div class="ml-auto flex items-center gap-2 lg:ml-0">
				...
				<Button
					...
					class="size-8"
					size="icon"
					onclick={() => navigateToPage((data.pagination?.page || 1) - 1)}
					disabled={(data.pagination?.page || 1) <= 1}
				>
				...
				<Button
					...
					size="icon"
					onclick={() => navigateToPage(data.pagination?.totalPages || 1)}
					disabled={(data.pagination?.page || 1) >= (data.pagination?.totalPages || 1)}
				>
...
{#snippet DataTableId({ row }: { row: Row<History> })}
	{row.getValue('invoice_id')}
{/snippet}

{#snippet DataTablePickupAddress({ row }: { row: Row<History> })}
	<div class="max-w-xs truncate text-sm leading-tight">
		{row.getValue('pickup_address')}
	</div>
{/snippet}

{#snippet DataTableTotalAmount({ row }: { row: Row<History> })}
	<div class="truncate px-1.5 text-left">
		{toIndonesianCurrency(row.getValue('total_amount'))}
	</div>
{/snippet}

{#snippet DataTableStatus({ row }: { row: Row<History> })}
...
{#snippet DataTablePaymentStatus({ row }: { row: Row<History> })}
	<Badge variant="outline" class="text-muted-foreground px-1.5 capitalize">
...
{#snippet DataTableAction({ row }: { row: Row<History> })}
	<Button
		variant="outline"
		size="sm"
		onclick={() => goto(`/orders/${row.getValue('invoice_id')}`)}
